...
LOG_DIR="/home/wins/Ethereum/fuzztest"
LOG_ENABLE=1 #enables output to the file
export PYTHONPATH=$PYTHONPATH:$PY_ETHEREUM_PATH
#options
	...
	if [ $FILL_WRONG == 0 ]; then
		...
		then
		 if [ $TEST_SUITE == "BlockChainTests" ]; then
			OUTPUT_GO="$($GO_ETHEREUM_PATH/bin/ethtest --test BlockTests --stdin "$TEST" 2> /dev/null)"
		 else
		 	OUTPUT_GO="$($GO_ETHEREUM_PATH/bin/ethtest --test $TEST_SUITE --stdin "$TEST" 2> /dev/null)"
		 fi
		 RESULT_GO=$?
		 if [ $TEST_DEBUG -eq 1 ]; then
			echo $OUTPUT_GO
